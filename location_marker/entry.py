    ...
    for line in help_msg_lines:
        ...
        if result is not None:
            help_msg_rtext.append(
                RText(line).c(RAction.suggest_command, result.group()).h('点击以填入 §7{}§r'.format(result.group())))
        else:
            help_msg_rtext.append(line)
    source.reply(help_msg_rtext)


def get_coordinate_color(dimension: int):
    return {
        0: RColor.green,
        -1: RColor.red,
        1: RColor.light_purple
    }.get(dimension, RColor.white)


def get_rtext(dimension: int) -> RTextBase:
    return RTextTranslation({
                            ...
                            }[dimension]).set_color(get_coordinate_color(dimension))


def get_coordinate_text(coord: Point, dimension: int):
    coord_text = RText('[{}, {}, {}]'.format(int(coord.x), int(coord.y), int(coord.z)),
                       get_coordinate_color(dimension))

    return coord_text.h(get_rtext(dimension))
...
def print_location(location: Location, printer: Callable[[RTextBase], Any], *, show_list_symbol: bool):
    ...
    text = RTextList(
        ...
        ' ',
        get_xearo_map_text(location.name, location.pos, location.dim),
        ' ',
        get_voxel_map_text(location.pos, location.dim),
        ' §7@§r ',
...
def get_xearo_map_text(name, pos, dim):
    command = "/xaero_waypoint_add:{}:{}:{}:{}:{}:6:false:0".format(name, name[0], int(pos.x), int(pos.y),
                                                                              int(pos.z))
    command += ':Internal_{}_waypoints'.format(get_dim_key(dim).replace('minecraft:', '').strip())
    return RText('[+X]', RColor.gold).h('§6Xaeros Minimap§r: 点击添加路径点').c(RAction.run_command, command)


def get_voxel_map_text(pos, dim):
    return RText('[+V]', RColor.aqua).h('§bVoxelmap§r: 点此以高亮坐标点, 或者Ctrl点击添加路径点').c(
        RAction.run_command, '/newWaypoint x:{}, y:{}, z:{}, dim:{}'.format(
            int(pos.x), int(pos.y), int(pos.z), get_dim_key(dim)
        ))
...
def show_location_detail(source: CommandSource, name):
    ...
    if loc is not None:
        ...
        source.reply(RTextList('坐标: ', get_coordinate_text(loc.pos, loc.dim)))
        ...
        source.reply(
            'VoxelMap路标(1.16+): [name:{}, x:{}, y:{}, z:{}, dim:{}]'.format(loc.name, x, y, z, get_dim_key(loc.dim)))
        # <Location Marker> xaero-waypoint:test:T:9987:71:9923:6:false:0:Internal-overworld-waypoints
        source.reply('<{}> xaero-waypoint:{}:{}:{}:{}:{}:6:false:0:Internal-{}-waypoints'.format(
            server_inst.get_self_metadata().name, loc.name, loc.name[0], x, y, z,
            get_dim_key(loc.dim).replace('minecraft:', '')))
    else:
        source.reply('未找到路标§b{}§r'.format(name))
            ...
            Literal('add').then(
                ...
                then(
                    Number('x').then(Number('y').then(Number('z').then(
                        ...
                        then(
                            GreedyText('desc').
                            runs(lambda src, ctx: add_location(src, ctx['name'], ctx['x'], ctx['y'], ctx['z'],
                                                               ctx['dim'], ctx['desc']))
                        )
                    )))
                )
